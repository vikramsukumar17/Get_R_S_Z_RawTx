...
def getSignableTxn(parsed):
    ...
    inputAddr = base58.b58decode_check(AOU_Utils.pubKeyToAddr(pub))
...
def verifyTxnSignature(txn):                    
    ...
    sig = AOU_Utils.derSigToHexSig(parsed[1][:-2])
...
def makeSignedTransaction(privateKey, outputTransactionHash, sourceIndex, scriptPubKey, outputs):
    ...
    sig = sk.sign_digest(s256, sigencode=ecdsa.util.sigencode_der) + '\01' 

    pubKey = AOU_Utils.privateKeyToPublicKey(privateKey)
...
class TestTxnUtils(unittest.TestCase):
    ...
    def test_makeSignedTransaction(self):
        ...
        privateKey = AOU_Utils.wifToPrivateKey("5Kb6aGpijtrb8X28GzmWtbcGZCG8jHQWFJcWugqo3MwKRvC8zyu") 
        ...
        signed_txn = makeSignedTransaction(privateKey,
            ...
            AOU_Utils.addrHashToScriptPubKey("133txdxQmwECTmXqAr9RWNHnzQ175jGb7e"),

            [[24321, 

            AOU_Utils.addrHashToScriptPubKey("1KKKK6N21XKo48zWKuQKXdvSsCf95ibHFa")],

             [20000,            AOU_Utils.addrHashToScriptPubKey("15nhZbXnLMknZACbb3Jrf1wPCD9DWAcqd7")]]
